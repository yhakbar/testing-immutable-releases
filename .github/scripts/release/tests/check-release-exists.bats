...
@test "writes exists=false and exits 0 when no release found" {
  ...
  grep -q "^is_published=false$" "$GITHUB_OUTPUT"
  grep -q "^release_id=$" "$GITHUB_OUTPUT"
}

@test "reports draft release" {
  export GH_STUB_RESPONSE='{"id":123,"draft":true,"upload_url":"https://uploads/repos/o/r/releases/123/assets{?name,label}","tag_name":"v1.0.0"}'
  run "$SCRIPT"
  [ "$status" -eq 0 ]
...
@test "reports published release" {
  export GH_STUB_RESPONSE='{"id":456,"draft":false,"upload_url":"https://uploads/repos/o/r/releases/456/assets{?name,label}","tag_name":"v1.0.0"}'
  run "$SCRIPT"
  [ "$status" -eq 0 ]
  grep -q "^exists=true$" "$GITHUB_OUTPUT"
  grep -q "^release_id=456$" "$GITHUB_OUTPUT"
  grep -q "^is_draft=false$" "$GITHUB_OUTPUT"
  grep -q "^is_published=true$" "$GITHUB_OUTPUT"
}
